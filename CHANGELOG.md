...
Автозапуск при загрузке теперь можно выключить.

* новый ключ `autostart` (по умолчанию `true`). При `false` модуль остаётся
  установленным и включённым в Magisk, но при загрузке не поднимает прокси
  и не ставит правила перехвата
* проверка живёт в `service.sh`, то есть только на пути загрузки. Ручной
  запуск - `dnscrypt-ctl start`, кнопка в приложении, плитка в шторке -
  работает как обычно и при выключенном автозапуске
* в журнал пишется строка о выключенном автозапуске - чтобы молчание
  после перезагрузки не выглядело поломкой
* убран осиротевший комментарий про зеркало в common.sh

# 2.3.1

Починен сброс `run_gid` в значение по умолчанию.

`dnscrypt-ctl set` требовал непустое значение (`[ -n "$3" ]`), а для `run_gid`
пустота — это и есть штатный сброс. В итоге выставить `net_admin` было можно,
а вернуть обратно — нет, ни из шелла, ни из приложения.

* проверяется факт передачи аргумента (`$# -ge 3`), а не его непустота;
* пустое значение разрешено только для `run_gid`, остальные ключи
  по-прежнему требуют значения;
* `dnscrypt-ctl set run_gid default` и `none` — псевдонимы пустоты для шелла,
  где пустой аргумент теряется между слоями кавычек.

Новая команда `dnscrypt-ctl reset`: возвращает весь `module.conf` к поставляемым
значениям, предварительно сохраняя прежние рядом в `.bak`. Раньше способа
сбросить настройки целиком не было вовсе.

# 2.3

Модуль больше ничего не хранит на внутренней памяти. Всё живёт в
`/data/adb/dnscrypt-proxy`, как у остальных модулей Magisk.

* убрана синхронизация с `/storage/emulated/0/dnscrypt-proxy`. Она была
  двусторонней и побеждал файл со свежим mtime, поэтому копия на общей
  памяти могла молча отменить только что выставленные настройки: достаточно
  было файлового менеджера, облачной синхронизации или сдвига часов
* убран ключ `mirror_config`; `dnscrypt-ctl set mirror_config` объясняет, что
  настройка удалена
* убраны команды `export` и `import`
* конфиг больше не ложится в общую память с правами 0644,
  где его читало любое приложение с доступом к хранилищу
* снята зависимость от credential-encrypted раздела, недоступного до
  первой разблокировки после загрузки

Старая папка `/storage/emulated/0/dnscrypt-proxy` не удаляется при обновлении -
чужие файлы молча сносить нельзя. Модуль её просто больше не трогает,
можно удалить вручную. При удалении модуля она по-прежнему чистится.

# 2.2.1

Починен `run_gid`: в 2.1.18-r3 и 2.2 он не работал ни при каком значении и уводил
резолвинг в отказ. Если вы его выставляли - обновитесь.

* `su` вызывался как `su 0 -g GROUP -c ...`, то есть опции шли после имени
  пользователя и уходили в шелл: `sh: -g: unknown option`, прокси падал с кодом 1
  в бесконечном цикле перезапусков
* Magisk su принимает только числовой GID и отвергает имя группы, поэтому
  задокументированное `run_gid=net_admin` не сработало бы и с верным порядком.
  Имена `net_admin`, `inet`, `net_raw` теперь переводятся в GID самим модулем
* перед запуском проверяется, что смена группы вообще возможна; если нет -
  в журнал уходит строка, а прокси стартует обычным способом. Настройка
  больше не может оставить систему без DNS
* при запуске через `su` в PID-файл писался pid посредника, а не прокси

# 2.2

Своя нумерация версий: она больше не повторяет версию dnscrypt-proxy, та указана
отдельно в описании модуля. Скрипты модуля с 2.1.18-r3 не менялись.

* приложение-менеджер DNSCrypt Manager: APK лежит в этом же релизе.
  Состояние прокси, выбор резолверов, правка конфига, журналы, плитка в шторке,
  проверка резолвинга без root
* README переписан: проект описан как самостоятельный, добавлен раздел
  благодарностей со ссылками на dnscrypt-proxy, d3cim, Magisk и разобранные
  по исходникам box_for_magisk и magisk-zapret2
...
* supervisor: экспоненциальная пауза вместо busy-loop при падении прокси
* данные перенесены в /data/adb/dnscrypt-proxy (работает до разблокировки экрана)
* копия конфига на внутренней памяти с двусторонней синхронизацией
* правила перехвата: своя цепочка nat/DNSCRYPT, идемпотентная привязка, route_localnet
* правила ставятся в late_start и восстанавливаются, если их снёс netd
* убрано глобальное отключение IPv6 (ломало раздачу Wi-Fi), IPv6-перехват опционален
* исключения из перехвата собираются из конфига, а не из захардкоженного IP
* переписан uninstall.sh: корректное ожидание, снятие правил, бэкап конфига
* добавлена утилита dnscrypt-ctl (status/json/set/restart/log/rules/import/export)
* убран updateJson: он вёл на апстрим и откатывал бы форк на 2.1.5
